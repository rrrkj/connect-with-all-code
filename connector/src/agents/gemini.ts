...
const logger = pino({ name: 'agent:gemini' });

const DEFAULT_TIMEOUT_MS = 3_600_000; // 60 minutes

/**
 * Antigravity agent runner — uses the Gemini CLI (`gemini`).
 */
export class GeminiRunner implements AgentRunner {
    private command: string;
    private timeoutMs: number;

    constructor(command: string = 'gemini', timeoutMs: number = DEFAULT_TIMEOUT_MS) {
    ...
    async execute(prompt: string, workspace: string): Promise<TaskResult> {
        ...
        try {
            ...
            logger.info({ taskId, workspace: resolvedWorkspace, promptLength: prompt.length }, 'Executing Gemini task');

            // Gemini CLI: -p for prompt, --yolo for auto-approve, -o json for structured output
            const args = [
                '-p', prompt,
                '--yolo',
                '-o', 'json',
            ...
            const durationMs = Date.now() - startTime;
            logger.error({ taskId, err: err.message }, 'Gemini task failed');
